...
// Represents the structure of a mission generated by the AI
...
class GeminiClient {
   ...
   */
  async generateMission(difficulty: 'Easy' | 'Medium' | 'Hard', language: string): Promise<GeneratedMission> {
    if (this.useMock) {
    ...
    try {
      ...
      const mission = JSON.parse(jsonString) as GeneratedMission;
   ...
   */
  async verifyCode(code: string, missionDescription: string, testCases: { input: string; expectedOutput: string }[], language: string): Promise<boolean> {
    if (this.useMock) {
